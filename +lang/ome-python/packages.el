...
(defun ome-python/post-init-company ()
  ;; backend specific
  (add-hook 'python-mode-local-vars-hook #'spacemacs//ome-python-setup-company)
...
(defun ome-python/init-live-py-mode ()
...
(defun ome-python/init-pip-requirements ()
  (use-package pip-requirements
    :defer t))

(defun ome-python/init-pippel ()
  (use-package pippel
    :defer t
...
(defun ome-python/init-py-isort ()
  (use-package py-isort
    :defer t
    :init
    (progn
      (add-hook 'before-save-hook 'spacemacs//ome-python-sort-imports)
      (spacemacs/set-leader-keys-for-major-mode 'python-mode
        "rI" 'py-isort-buffer))))

(defun ome-python/pre-init-pyenv-mode ()
  (add-to-list 'spacemacs--python-pyenv-modes 'python-mode))
(defun ome-python/init-pyenv-mode ()
       ...
       (`on-visit
        (dolist (m spacemacs--python-pyenv-modes)
          (add-hook (intern (format "%s-hook" m))
                    'spacemacs//ome-pyenv-mode-set-local-version)))
       (`on-project-switch
        (add-hook 'projectile-after-switch-project-hook
                  'spacemacs//ome-pyenv-mode-set-local-version)))
      ;; setup shell correctly on environment switch
      (dolist (func '(pyenv-mode-set pyenv-mode-unset))
        (advice-add func :after 'spacemacs/ome-python-setup-everything))
      (spacemacs/set-leader-keys-for-major-mode 'python-mode
        "vu" 'pyenv-mode-unset
        "vs" 'pyenv-mode-set))))

(defun ome-python/pre-init-pyvenv ()
  (add-to-list 'spacemacs--python-pyvenv-modes 'python-mode))
(defun ome-python/init-pyvenv ()
  (use-package pyvenv
    :defer t
    :init
    (progn
      (pcase python-auto-set-local-pyvenv-virtualenv
        (`on-visit
         (dolist (m spacemacs--python-pyvenv-modes)
           (add-hook (intern (format "%s-hook" m))
                     'spacemacs//ome-pyvenv-mode-set-local-virtualenv)))
        (`on-project-switch
         (add-hook 'projectile-after-switch-project-hook
                   'spacemacs//ome-pyvenv-mode-set-local-virtualenv)))
      (dolist (m spacemacs--python-pyvenv-modes)
...
(defun ome-python/init-pylookup ()
  ...
  (use-package python
    ...
    (progn
      (spacemacs/register-repl 'python
                               'spacemacs/ome-python-start-or-switch-repl "python")
      (add-hook 'inferior-python-mode-hook
                'spacemacs//ome-inferior-python-setup-hook)
      (spacemacs/add-to-hook 'python-mode-hook
                             '(spacemacs//ome-python-setup-backend
                               spacemacs//ome-python-default))
      ;; call `spacemacs//ome-python-setup-shell' once, don't put it in a hook
      ;; (see issue #5988)
      (spacemacs//ome-python-setup-shell))
    :config
    (progn
      ...
      (spacemacs/set-leader-keys-for-major-mode 'python-mode
        "'"  'spacemacs/ome-python-start-or-switch-repl
        "cc" 'spacemacs/ome-python-execute-file
        "cC" 'spacemacs/ome-python-execute-file-focus
        "db" 'spacemacs/ome-python-toggle-breakpoint
        "ri" 'spacemacs/ome-python-remove-unused-imports
        "sB" 'spacemacs/ome-python-shell-send-buffer-switch
        "sb" 'python-shell-send-buffer
        "sF" 'spacemacs/ome-python-shell-send-defun-switch
        "sf" 'python-shell-send-defun
        "si" 'spacemacs/ome-python-start-or-switch-repl
        "sR" 'spacemacs/ome-python-shell-send-region-switch
...
(defun ome-python/post-init-semantic ()
...
(defun ome-python/pre-init-smartparens ()
...
(defun ome-python/post-init-smartparens ()
  (add-hook 'inferior-python-mode-hook 'smartparens-mode))

(defun ome-python/post-init-stickyfunc-enhance ()
  (add-hook 'python-mode-hook 'spacemacs/load-stickyfunc-enhance))

(defun ome-python/init-yapfify ()
  (use-package yapfify
    :defer t
    :init
    (progn
